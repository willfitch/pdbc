...
/* {{{ proto string confirm_pdbc_compiled(string arg)
   Return a string to confirm that the module is compiled in */
PHP_FUNCTION(confirm_pdbc_compiled)
{
	char *arg = NULL;
	size_t arg_len, len;
	zend_string *strg;
	...
	strg = strpprintf(0, "Congratulations! You have successfully modified ext/%.78s/config.m4. Module %.78s is now compiled into PHP.", "pdbc", arg);
...
/* {{{ pdbc_functions[]
 *
 * Every user visible function must have an entry in pdbc_functions[].
 */
const zend_function_entry pdbc_functions[] = {
	PHP_FE(confirm_pdbc_compiled,	NULL)		/* For testing, remove later. */
	PHP_FE_END	/* Must be the last line in pdbc_functions[] */
};
/* }}} */
